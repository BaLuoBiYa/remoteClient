...
function saveSetting(key: keyof Settings, value: unknown) {
    try {
        const raw = localStorage.getItem(STORAGE_KEY);
...
async function initParams() {
    ...
    if (stored) {
        ...
        for (const [k, v] of Object.entries(stored)) {
            invoke("update_setting", { key: k, value: v }).catch(console.error);
        }
    }
...
// ---------- 首次启动 WebSocket URL 弹窗 ----------
const WS_URL_FIRST_RUN_KEY = "dog-ws-url-first-run";

function hasStoredUrl(): boolean {
    try {
        const raw = localStorage.getItem(STORAGE_KEY);
        if (!raw) return false;
...
function showWSModal(): Promise<string | null> {
    return new Promise((resolve) => {
        ...
        const onSubmit = () => {
            ...
            invoke("update_setting", { key: "ws_url", value: url }).catch(console.error);
